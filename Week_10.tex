...
\item AR($p$) model (autoregressive model of order $p$): the modeled variable depends linearly on its own previous values and a stochastic term
\end{itemize}
\medskip
AR($p$): $y_t = c + \rho_1 y_{t-1} + \rho_2 y_{t-2} + \dots + \rho_p y_{t-p} + u_t = c + \sum_{i=1}^p \rho_i y_{t-i} + u_t$ \\
\bigskip
\underline{\textbf{VAR models}} generalize the univariate autoregressive model (AR model) by allowing for more than one evolving variable:\\ $y_t$ becomes $\bm{y}_t$, where $\bm{y}_t^T = (y_{1t}, y_{2t}, \dots , y_{mt})$\\
\begin{itemize}
...
\vspace*{-1.3mm}
	...
	\end{itemize}
\textbf{Decide whether to include any deterministic variables}
\vspace*{-1.5mm}
\begin{itemize}
\item Trends, time dummies, seasonal terms, even exogenous regressors (e.g. oil/energy prices).\{often, VAR models augmented by deterministic regressors are called VARX($p$) models\}
\end{itemize}
\end{frame}
%---------------------------------
\begin{frame}{VAR: Model setup and testing}
...
\textbf{Granger Causality tests are applied for setup verification}\\
	...
	\item Different types of GC tests exist.
...
\item Sometimes the lag length corresponds to the data, such that quarterly data have $4$ lags, monthly data have $12$ lags, etc.
...
\bigskip
In a VAR($p)$ model, $\bm{\Sigma}_u = \textit{cov}(\bm{u}_t)$ is the white noise covariance matrix and it may be expressed as $\bm{\Sigma}_u =\bm{PP'}$ where $\bm{P}$ is a lower triangular matrix (assumptions on $\bm{\Sigma}_u$ apply).\\
\medskip
...
\medskip
$y_{1t} \sim I(1)$ nominal F/X rate between two currencies (domestic/foreign), \\ 
$y_{2t} \sim I(1)$ describes domestic inflation (e.g. CPI), \\
$y_{3t} \sim I(1)$ describes foreign inflation. \\
\medskip
...
\tiny
\textcolor{Blue}{
\url{https://www.rmetrics.org/files/Meielisalp2007/Presentations/Pfaff.pdf} \\
\vspace{0.2cm}
\tiny \url{https://www.rmetrics.org/files/Meielisalp2007/Presentations/Breymann.pdf} \\
\vspace{0.5cm}
\tiny
