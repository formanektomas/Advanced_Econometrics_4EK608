...
\vspace*{-1.5mm}
\begin{itemize}
\item In most empirical econometric models, we would use up to six variables (roughly). However, there is no theoretical upper limit for $m$, given enough observations are available.
\end{itemize}
...
\vspace*{4mm}
$F = \frac{(\textit{SSR}_R - \textit{SSR}_{UR})}{q(\textit{SSR}_{UR})}(T-k) \!\Duck{H_0}\! F(q ; T - k)$\\
{\scriptsize where\\
\vspace*{1.5mm}
