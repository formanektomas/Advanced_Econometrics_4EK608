...
\end{frame}
%---------------------------------------------
\begin{frame}{IVR estimator is biased and consistent}
\begin{align*}
    ...
    \end{align*}
\begin{itemize}
    \item In small (finite) samples, $[(\bm{Z}^{\prime}\bm{X})^{-1} \bm{Z}^{\prime}\bm{u}] \neq \bm{0}$ because of endogeneity in $\bm{X}$.
    \medskip
...
\medskip
\textbf{\underline{DWH test motivation:}} \\If $z_j$ are proper instruments (uncorrelated with $u$), then $y_2$ is endogenous (correlated with $u$) if and only if $\varepsilon$ is correlated with $u$. \\
\medskip
\begin{itemize}
