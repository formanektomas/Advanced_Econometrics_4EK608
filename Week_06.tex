...
\node[mynode] at (4.7,3) (Box){Each year, we draw $500$ individuals at random. Individual respondents are not followed. Total observations: $N \times T = 1.500$};
...
%---------------------------------
\section{Policy analysis with pooled cross sections}
...
\item Estimate average effect (\& st. error)
\end{itemize}
\end{minipage}
};
...
\node[mynode] at (3,-0.3) (delta*){$\delta_1$ measures the difference in $x_1$ slope for $\textit{ind1}$ };
...
$$y_{it} = \alpha_1 \mytikzmark{ind1}{\boxed{\textit{ind}1_{it}}} + \alpha_2 \mytikzmark{ind2}{\boxed{\textit{ind}2_{it}}} + \dots + \alpha_N \mytikzmark{indN}{\boxed{\textit{ind}N_{it}}} + \beta_1 x_{it1} + \dots + \beta_k x_{itk} + u_{it}$$
\begin{itemize}
\item $\hat{\bm{\beta}}_{LSDV}$ is identical to $\hat{\bm{\beta}}_{FE}$ (explained next) - it is a consistent estimator of $\bm{\beta}$, even if we hold $T$ fixed and $N \rightarrow \infty$.
...
\node[mynode] at (5.5,4) (dummy){Dummy equals $1$, only if the observation at time $t$ relates to the appropriate C-S unit.};
...
\medskip
Alternative notation: $\ddot{y}_{it} = \bm{\ddot{x}_{it} \beta} + \ddot{u}_{it}$; where $\ddot{y}_{it} = y_{it} - \overline{y}_{i}$, etc.\\
\medskip
