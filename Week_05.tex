...
\item If $\hat{\theta}$ is an unbiased estimator of $\theta$ and $\textit{plim}(\textnormal{var}(\hat{\theta}))=0$ \\i.e. [ $\textit{var}(\hat{\theta}) \rightarrow 0 \textnormal{ as } n \rightarrow \infty$], then $\textit{plim}(\hat{\theta}) = \theta$. 
...
\begin{itemize}
    ...
    \item \textbf{Method of moments estimator} $\hat{\bm{\theta}}_{MM}$ minimizes:
    $$
