...
\medskip
\item The (linear) quantile model can be defined as $Q[y|\bm{x}, q]=\bm{x\beta}$, such that $\textnormal{Prob}[y \le \bm{x\beta}_q|\bm{x}]=q$, $0<q<1$ \\where $q$ denote the $q$-th quantile.
\medskip
...
Gauss’s method (OLS) supplanted LAD at its origination; OLS is vastly easier to compute.
\item Different software packages use a variety of iterative numerical algorithms as quantile regression estimators.
