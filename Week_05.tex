...
\medskip
\item The (linear) quantile model can be defined as $Q[y|\bm{x}, q]=\bm{x}^{\prime}\bm{\beta}_q$, such that $\textnormal{Prob}[y \le \bm{x}^{\prime}\bm{\beta}_q|\bm{x}]=q$, $0<q<1$ \\where $q$ denote the $q$-th quantile.
\medskip
...
$$
\item LAD estimator predates OLS (itself older than 200 years). So far, QREG and LAD has seen little use in econometrics, as OLS is vastly easier to compute.
\item Different software packages use a variety of iterative optimization algorithms for QREG/LAD estimation.
\item Linear programming can also be used for finding QREG estimates (Koenkerr and Bassett (around 1980)).
\end{itemize}
\end{frame}
%---------------------------------------------
...
\bigskip
\begin{enumerate}
    \item $\textnormal{wage}_i = \beta_0 + u_i$
    \item $\textnormal{wage}_i = \beta_0 + \beta_1 \textnormal{female}_i + u_i$
        \item $\textnormal{wage}_i = \beta_0 + \beta_1 \textnormal{female}_i + \beta_2 \, \textnormal{exper}_i + u_i$
\end{enumerate}
\bigskip
